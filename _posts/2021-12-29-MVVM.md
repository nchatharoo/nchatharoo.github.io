...
![MVC-MVVM](/Users/nadheer/Documents/nchatharoo.github.io/images/2021-12-29-MVC-MVVM.png)
...
![Capture d’écran 2021-12-30 à 09.01.26](/Users/nadheer/Documents/nchatharoo.github.io/images/2021-12-29-MVVM-Binder.png)

But, in UIKit, MVVM won't reduce a lot of boilerplate code since there’s no automatic binding. Still, ViewModels are an good idea to create a reusable logic between UI and core components and a better architectural separation. It’s recommended for ViewModels to be platform and framework-agnostic so that you can reuse them on multiple platforms.
...
You can see that the ViewController is communicating with the ModelLoader, it also manage the model loading state with the var onRefresh and it create and configure a UIRefreshControl.
...
final class ViewController: NSObject {
    ...
    init(loader: ModelLoader) {
        self.viewModel = ViewModel(loader: loader)
    }
...
In the `binded` function we set the `onLoadingStateChange` and pass a boolean, and just need to weakify view, since we do not need a reference to self.
